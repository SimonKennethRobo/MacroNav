...
def _create_tiny_vit(variant, pretrained=False, **kwargs):
    ...
    cfg = dict(
        url=_checkpoint_url_format.format(f"{variant_without_img_size}_{pretrained_type}"),
    ...
    if timm.__version__ >= "0.6":
        return build_model_with_cfg(
            TinyViT, variant, pretrained, pretrained_cfg=cfg, pretrained_filter_fn=_pretrained_filter_fn, **kwargs
        )
    else:
        return build_model_with_cfg(
            TinyViT, variant, pretrained, default_cfg=cfg, pretrained_filter_fn=_pretrained_filter_fn, **kwargs
        )
